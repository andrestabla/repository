...
export const PdfReportData = React.forwardRef<HTMLDivElement, PdfReportDataProps>(({ state, scoring, reports, radarData }, ref) => {
    ...
    const markdownComponents: any = {
        p: ({ node, ...props }: any) => <p className="pdf-block mb-4" {...props} />,
        ul: ({ node, ...props }: any) => <ul className="pdf-block mb-4 list-disc pl-4 ml-4" {...props} />,
        ol: ({ node, ...props }: any) => <ol className="pdf-block mb-4 list-decimal pl-4 ml-4" {...props} />,
        h1: ({ node, ...props }: any) => <h1 className="pdf-block mt-6 mb-2 font-bold text-lg" {...props} />,
        h2: ({ node, ...props }: any) => <h2 className="pdf-block mt-5 mb-2 font-bold text-md" {...props} />,
        h3: ({ node, ...props }: any) => <h3 className="pdf-block mt-4 mb-2 font-bold" {...props} />,
        li: ({ node, ...props }: any) => <li className="mb-1" {...props} />
    };

    return (
        <div ref={ref} className="bg-white p-12 text-[#0f172a]" style={{ width: '210mm' }}>

            {/* COVER HEADER */}
            <div className="pdf-block flex items-center gap-4 mb-16 border-b border-slate-200 pb-8">
            ...
            {/* EXECUTIVE SUMMARY */}
            <div className="pdf-block flex items-center gap-12 mb-16 bg-slate-50 p-10 rounded-3xl border border-slate-100">
                <div>
                    <div className="w-56 h-56 rounded-full border-[12px] border-white flex flex-col items-center justify-center bg-slate-50 shadow-sm mx-auto">
            ...
            {/* MAPPING GLOBAL RADAR */}
            <div className="pdf-block mb-16">
            ...
            {/* BLOCK: GLOBAL AI ANALYSIS */}
            {reports['all'] && (
                <div className="mb-16">
                    <h3 className="pdf-block text-2xl font-black text-slate-900 mb-6 border-b border-indigo-100 pb-4 text-indigo-900">AI Executive Consultant: Análisis Global</h3>
                    <div className="prose prose-slate prose-sm max-w-none text-justify text-slate-700">
                        <ReactMarkdown components={markdownComponents}>{reports['all']}</ReactMarkdown>
                    </div>
                </div>
            )}
            ...
            {/* COMPETENCIES TABLE */}
            <div className="mb-16">
                <h3 className="pdf-block text-xl font-black text-slate-900 mb-6 border-b border-slate-200 pb-4">Desglose por Competencia (Score 1-5)</h3>
                <table className="w-full text-left text-sm">
                    <thead className="pdf-block text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-200 bg-white">
                        <tr>
                    ...
                    <tbody className="divide-y divide-slate-100">
                        {scoring.compList.map((c: any, i: number) => {
                            ...
                            return (
                                <tr key={i} className="pdf-block bg-white">
            ...
            {pillars.map(p => {
                ...
                return (
                    <div key={p} className="mb-16">
                        <div className="pdf-block flex items-center justify-between border-b-4 border-indigo-600 pb-6 mb-10">
                        ...
                        <div className="pdf-block grid grid-cols-2 gap-10 mb-10">
                        ...
                        {pReport ? (
                            <div className="bg-indigo-50/50 p-8 rounded-3xl border border-indigo-100 pb-2">
                                <h3 className="pdf-block text-xl font-black text-indigo-900 mb-6">AI Executive Consultant: Deep Dive</h3>
                                <div className="prose prose-slate prose-sm max-w-none text-justify text-slate-700">
                                    <ReactMarkdown components={markdownComponents}>{pReport}</ReactMarkdown>
                                </div>
                            </div>
                        ) : (
                            <div className="pdf-block bg-slate-50 p-8 rounded-3xl border border-slate-100 text-center">

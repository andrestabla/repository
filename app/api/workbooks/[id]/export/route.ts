...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        let html = ""
        try {
            const { WORKBOOK_TEMPLATES } = require('@/lib/workbook-templates')
            const w = workbook as any
            if (w.type && WORKBOOK_TEMPLATES[w.type]) {
                html = WORKBOOK_TEMPLATES[w.type].exportTemplate(workbook)
            }
        } catch (e) {
            console.warn("[Export] Specialized template could not be loaded", e)
        }

export const WORKBOOK_V2_EDITORIAL = {
    classes: {
        shell: 'min-h-screen bg-[#f4f7fb] text-[#0f172a]',
        toolbar: 'sticky top-0 z-40 border-b border-slate-200 bg-white/90 backdrop-blur-xl shadow-[0_8px_24px_rgba(15,23,42,0.06)]',
        toolbarInner: 'max-w-[1280px] mx-auto px-5 md:px-8 py-4 flex flex-wrap items-center gap-3',
        backButton:
        ...
        sidebar:
            'rounded-2xl border border-slate-200/90 bg-white p-4 lg:sticky lg:top-24 shadow-[0_12px_28px_rgba(15,23,42,0.06)]',
        sidebarTitle: 'text-[11px] font-bold uppercase tracking-[0.2em] text-slate-500 mb-3',
        bottomNav: 'rounded-2xl border border-slate-200 bg-white p-4 md:p-5 shadow-sm flex items-center justify-between',
        bottomNavPrev:
            'inline-flex items-center gap-2 rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100 transition-colors disabled:opacity-40 disabled:cursor-not-allowed',
        bottomNavNext:
            'inline-flex items-center gap-2 rounded-lg bg-slate-900 text-white px-4 py-2 text-sm font-semibold hover:bg-slate-800 transition-colors disabled:opacity-40 disabled:cursor-not-allowed',
        progressPill:
